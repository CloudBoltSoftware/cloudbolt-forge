"""Verify that the Azure Queue BP is working as expected.
"""
...
NEW_RESOURCE_NAME = "cl-cit-azure-queue-test-" + str(uuid.uuid4())[:8]
# All Azure Storage Queues must be deployed to an Azure Resource Group. A change to the
# PAYLOAD may also require that the Resource Group is manually created in the Azure Portal.
_AZURE_RESOURCE_GROUP = "cit_content_library_azure_storage"
_AZURE_STORAGE_ACCOUNT = "storedcc57eetgmtck"

## CloudBolt Environment Variables
API_CLIENT_CI = "CIT API Client"
GROUP = Group.objects.get(name="Temp Group", id=2)
BLUEPRINT = ServiceBlueprint.objects.get(name__startswith="Azure Queues", id=68)
        ...
        "deploy-items": [
            {
                ...
                "blueprint-items-arguments": {
                    f"build-item-{_BUILD_ITEM.name}": {
                        "parameters": {
                            "azure-queue-name-a957": NEW_RESOURCE_NAME,
                            "env-id-a957": str(ENVIRONMENT.id),
                            "resource-group-a957": _AZURE_RESOURCE_GROUP,
                            "storage-account-a957": _AZURE_STORAGE_ACCOUNT,
                        }
                    }
                },
...
def run(job, *args, **kwargs):
    ...
    set_progress(f"Ordering with Blueprint {bp.name} and id {bp.id}")
    ...
    set_progress(f"DELETE RESOURCE {rce} FROM REMOTE")
