...
BP_PAYLOAD = """
{
    ...
    "items": {
        "deploy-items": [
            {
                "blueprint": "/api/v2/blueprints/BP-m87cola0/",
                "blueprint-items-arguments": {
                    "build-item-Create Google Cloud Function": {
                        "parameters": {
                            "available-memory-mb-a389": "128",
                            "bucket-to-store-sourcecode-a389": "cloudbolt-content-development.appspot.com",
                            "enter-sourcecode-or-bucket-url-a389": "SourceCode",
                            "entry-point-a389": "greetings",
                            "env-id-a389": "74",
                            "function-name-a389": "greetings",
                            "runtime-a389": "python37",
                             "source-code-a389": "def say_hi(request)return 'Say Hi'"
                        }
                    }
                },
                "resource-name": "Google Cloud Function",
                "resource-parameters": {}
            }
        ]
    },
...
def test_delete_resource(client, resource):
    delete = json.loads(client.post(
        '/api/v2/resources/{}/{}/actions/1/'.format(resource.resource_type.name, resource.id)))
...
def run(job, *args, **kwargs):
    ...
    # Since Google cloud takes sometime before the function can be available for
    # Delition, sleep for a few seconds
    time.sleep(60)
