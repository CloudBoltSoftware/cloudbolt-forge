...
BP_PAYLOAD = """
{
    ...
    "items": {
        "deploy-items": [
            {
                "blueprint": "/api/v2/blueprints/BP-pox4cfzy/",
                "blueprint-items-arguments": {
                    "build-item-Azure Security Rules Build": {
                        "parameters": {
                            "access-a418": "Allow",
                            "azure-network-security-group-name-a418": "cittestrule001",
                            "destination-address-prefix-a418": "*",
                            "destination-port-range-a418": "*",
                            "direction-a418": "Inbound",
                            "env-id-a418": "89",
                            "priority-a418": "1899",
                            "protocol-a418": "Tcp",
                            "resource-group-a418": "cit-test-l-0034",
                            "source-address-prefix-a418": "*",
                            "source-port-range-a418": "*"
                        }
                    }
                },
                "resource-name": "Azure Security Rules",
                "resource-parameters": {}
            }
        ]
    },
...
NEW_RESOURCE_NAME = "Azure Security rule - cittestrule001 Inbound"
...
def run(job, *args, **kwargs):
    ...
    resource = bp.resource_set.filter(name__icontains=NEW_RESOURCE_NAME, lifecycle='ACTIVE').first()
