...
BLUEPRINT = ServiceBlueprint.objects.get(name__startswith="Azure Maria DB", id=55)

# Create a unique resource name in case of overlap between prior failures.
NEW_RESOURCE_NAME = "cl-cit-azure-maria-db-test-" + str(uuid.uuid4())[:8]
...
PAYLOAD = {
    ...
    "items": {
        "deploy-items": [
            {
                "blueprint": "/api/v2/blueprints/BP-qgw54ar3/",
                "blueprint-items-arguments": {
                    "build-item-Build a Maria DB on Azure": {
                        "parameters": {
                            "database-name-a254": NEW_RESOURCE_NAME,
                            "env-id-a254": "8",
                            "resource-group-a254": "content_library_mariadb_test",
                            "server-password-a254": "dJTRAiTier7Cdm0sfhcDOQp437^kJU!fGWuC7$g4kYRv*WTK@mC",
                            "server-username-a254": "testuser",
                        }
                    }
                },
                "resource-name": "Azure Maria DB",
                "resource-parameters": {},
            }
        ]
    },
