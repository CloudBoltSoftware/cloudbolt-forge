...
BLUEPRINT = ServiceBlueprint.objects.get(name__startswith="Azure SQL Database", id=14)

NEW_RESOURCE_NAME = "cl-cit-azure-sql-" + str(uuid.uuid4())[:8]
...
PAYLOAD = {
    ...
    "items": {
        "deploy-items": [
            {
                "blueprint": "/api/v2/blueprints/BP-qa8n6isn/",
                "blueprint-items-arguments": {
                    "build-item-Create Azure SQL": {
                        "parameters": {
                            "database-name-a116": NEW_RESOURCE_NAME,
                            "env-id-a116": "8",
                            "resource-group-a116": "content_library_sql_db_test",
                            "server-password-a116": "thIS@123RESDasd",
                            "server-username-a116": "rubyththt",
                        }
                    }
                },
                "resource-name": "Azure SQL Database",
                "resource-parameters": {},
            }
        ]
    },
...
def test_order_blueprint(client):
    ...
    result = wait_for_order_completion(client, order_id, 1200, 10)
    if result != 0:
