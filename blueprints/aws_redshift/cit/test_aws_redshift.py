...
BP_PAYLOADS = ["""
{
    ...
    "items": {
        "deploy-items": [
            {
                "blueprint": "/api/v2/blueprints/BP-1mela6ej/",
                "blueprint-items-arguments": {
                    "build-item-Create AWS Redshift": {
                        "parameters": {
                            "ClusterIdentifier-a274": "testwalec",
                            "cluster-type-a274": "single-node",
                            "env-id-a274": "191",
                            "master-password-a274": "CloudB0lt##",
                            "master-username-a274": "cloudbolt",
                            "node-type-a274": "dc1.large"
                        }
                    }
                },
                "resource-name": "AWS Redshift",
                "resource-parameters": {}
            }
        ]
    },
...
def run(job, *args, **kwargs):
    ...
    for resources_name in resources_names:
        bp.resource_set.filter(name__iexact=resources_name).delete()

    # Run blueprint synchronization in case a redshift cluster has been created online
    bp.sync_resources()

    for resources_name in resources_names:
        resource_exists = bp.resource_set.filter(name__iexact=resources_name, lifecycle='ACTIVE').exists()
        if resource_exists:
            # Delete the resource
            resource = bp.resource_set.get(name__iexact=resources_name, lifecycle='ACTIVE')
            test_result = test_delete_resource(client, resource)
            if test_result != "SUCCESS":
                # The resource might only be available locally
                resource.delete()
            else:
                # Deleting the cluster online takes time so we can sleep for a few minutes
                time.sleep(600)
    ...
    # Wait for resource to be fully created
    set_progress("### Waiting for RESOURCE to be fully created ###", tasks_done=1)
    time.sleep(600)
    ...
    for resources_name in resources_names:
        ...
        if test_result != "SUCCESS":
            return "FAILURE", "Unable to delete the redshift cluster", f"{test_result}"
