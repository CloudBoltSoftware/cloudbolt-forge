...
SQL_VALID_REGIONS = [
    'northamerica-northeast1', 'us-central', 'us-central1', 'us-east1', 'us-east4', 'us-west1', 'us-west2',
    'southamerica-east1', 'europe-north1', 'europe-west1', 'europe-west2', 'europe-west3', 'europe-west4',
    'europe-west6', 'asia-east1', 'asia-east2', 'asia-northeast1', 'asia-northeast2', 'asia-south1', 'asia-southeast1',
...
def run(job=None, logger=None, **kwargs):
    ...
    job.set_progress('Connecting to Google Cloud through service account email {}'.format(account_info["client_email"]))
    ...
    try:
        ...
        if 'items' in inst_data:
            ...
            if instance_name in instance_names:
                return "ERROR", 'Server instance "%s" already exists' % instance_name, ''
    except HttpError as e:
        client_username = account_info["client_email"].split("@")[0]
        return "ERROR", "Server instance {instance_name} could not be created ({reason}), make sure that this ResourceHandler's service account ({service_account_name}) is given the Cloud SQL Admin Permission".format(instance_name=instance_name, reason=str(e), service_account_name=client_username), e
    ...
    result = client.databases().insert(project=project, instance=instance_name, body=body).execute()
    assert result['status'] == 'DONE'

    set_progress('Database %s is now available on instance: %s' % (db_name, instance_name))
