...
def discover_resources(**kwargs):
    ...
    existing_bps = ServiceBlueprint.objects.filter(name__contains='Google MySQL Database')

    for bp in existing_bps:
        for resource in bp.resource_set.filter(lifecycle='ACTIVE'):
            existing_instances.add((resource.gcp_sql_instance_name,resource.gcp_sql_project))
    ...
    for project in GCPProject.objects.filter(imported=True):
        ...
        set_progress('Connecting to Google Cloud through service account email {}'.format(client_email))
        ...
        try:
            instances = client.instances().list(project=project.id).execute().get('items', None)
            if instances:
                for instance in instances:
                    set_progress('Found Database named {}: checking to see if the instance already exists on CB'.format(instance['name']))
            ...
            set_progress('Could not list sql servers for {}, error message is as follows:{}'.format(project.name,e))
