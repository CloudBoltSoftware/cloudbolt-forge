...
def run(job, logger=None, **kwargs):
    ...
    try:
        ...
        try:
            account_info = json.loads(rh.gcp_projects.get(gcp_id=project).service_account_info)
        except Exception:
            account_info = json.loads(
                rh.gcp_projects.get(gcp_id=project).service_account_key
            )
        ...
        gcp_project_id = rh.gcp_projects.get(gcp_id=project).gcp_id
        # It takes awhile for the DB to be deleted
        client.instances().delete(project=gcp_project_id, instance=instance_name).execute()
        
        # Verify that instance was deleted:
        inst_data = client.instances().list(project=gcp_project_id).execute()
