...
BP_PAYLOADS = ["""
{
    ...
    "items": {
        "deploy-items": [
            {
                "blueprint": "/api/v2/blueprints/BP-azlflrc7/",
                "blueprint-items-arguments": {
                    "build-item-AWS Elastic File System Create": {
                        "parameters": {
                            "encrypted-a141": "False",
                            "env-id-a141": "6",
                            "performance-mode-a141": "generalPurpose"
                        }
                    }
                },
                "resource-name": "Amazon Elastic File System",
                "resource-parameters": {}
            }
        ]
    },
...
def run(job, *args, **kwargs):
    ...
    for order_id in orders_ids:
        ...
        efs_file_system_id = resource.efs_file_system_id
        ...
        resources = bp.resource_set.filter(name__istartswith="fs-", lifecycle='ACTIVE', blueprint=bp)

        set_progress("### DELETING RESOURCE FOR BLUEPRINT ###", tasks_done=2)

        [test_delete_resource(client, resource) for resource in resources if
         resource.efs_file_system_id == efs_file_system_id]
