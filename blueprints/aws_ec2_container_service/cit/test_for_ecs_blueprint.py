...
BP_PAYLOAD = """
{
    ...
    "items": {
        "deploy-items": [
            {
                "blueprint": "/api/v2/blueprints/72/",
                "blueprint-items-arguments": {
                    "build-item-Create AWS EC2 Container Service": {
                        "parameters": {
                            "cluster-name-a329": "awsecscit19",
                            "env-id-a329": "21"
                        }
                    }
                },
                "resource-name": "AWS EC2 Container Service",
                "resource-parameters": {}
            }
        ]
    },
...
def run(job, *args, **kwargs):
    ...
    resources = bp.resource_set.filter(name=NEW_RESOURCE_NAME, lifecycle='ACTIVE')
    resource = [res for res in resources if res.cluster_name == NEW_RESOURCE_NAME][0]
