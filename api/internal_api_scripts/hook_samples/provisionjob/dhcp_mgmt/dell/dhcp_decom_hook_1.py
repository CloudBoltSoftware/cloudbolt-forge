...
def delete_reservation(job, srv, logger=None):
    ...
    clean_hostname = re.split("\.", hostname)[0]

    nics = srv.nics.all()
    if not nics:
        return "FAILURE", "", "No NICs attached to server VM."
    domain = nics[0].network.dns_domain

    if not ip:
        ip = getattr(srv, "sc_nic_0_ip", None)

    msg = "Now working on server '{}'".format(clean_hostname)
    ...
    remote_cmd = (
        "ssh root@{dhcp_srv} "
        "'/etc/config_vm_network rem {hostname} {interface} {domain} {mac} {ip}'".format(
            dhcp_srv=dhcp_server_ip,
...
def run(job, logger=None):
    ...
    progmsg = "Removal of DHCP reservations completed successfully"
    job.set_progress(progmsg, logger=logger)
